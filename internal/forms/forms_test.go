...
func TestForm_Valid(t *testing.T) {
	...
	isValid := form.Valid()
	if !isValid {
		t.Error("gor inavlid when should have been valid")
	}
}
...
func Test_MinLenght(t *testing.T) {
	r := httptest.NewRequest("POST", "/whatever", nil)
	form := New(r.PostForm)
	...
	if form.Valid() {
		t.Error("form shows min lenth for non-existent field")
	}

	postedValues = url.Values{}
	postedValues.Add("another_field", "afdfd")
	form = New(postedValues)
