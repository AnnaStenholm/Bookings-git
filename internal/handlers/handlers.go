...
import (
	...
	"github.com/AnnaStenholm/bookings/helpers"
	"github.com/AnnaStenholm/bookings/internal/config"
	"github.com/AnnaStenholm/bookings/internal/driver"
	"github.com/AnnaStenholm/bookings/internal/forms"
	"github.com/AnnaStenholm/bookings/internal/models"
	"github.com/AnnaStenholm/bookings/internal/render"
	"github.com/AnnaStenholm/bookings/internal/repository"
	"github.com/AnnaStenholm/bookings/internal/repository/dbrepo"
...
// NewHandlers sets the repository fo the handlers
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	//send notifications - first to guests
	...
	htmlMessage = fmt.Sprintf(`
	<strong>Reservation has been made for %s from %s to %s.
...
// Shows all reservation in admin tool
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		//Get the block map from the session. Loop through map, if we have an entry that does not
		//exist in our posted data, and if the restrictions id > 0, then it is a block that we have to remove
		curMap := m.App.Session.Get(r.Context(), fmt.Sprintf("block_map_%d", x.ID)).(map[string]int)
		for name, value := range curMap {
			//ok will be false if its value is not in the map
